...
##############################################################################
# This will update Pheme 0.17.6 Stable to 0.18.5 Stable
...
/var/pheme/www/bin/console pheme:backup $installerHome/tools/pheme/update/backup/$FALLBACK_VERSION.zip
echo -e "Backup of $FALLBACK_VERSION is located in $installerHome/tools/pheme/update/backup/$FALLBACK_VERSION.zip\n"
...
curl -s -o $PHP_POOL_DIR/php.ini https://raw.githubusercontent.com/ashd0wn/Pheme-Ubuntu/$newVersion/web/php/php.ini
curl -s -o $PHP_POOL_DIR/www.conf https://raw.githubusercontent.com/ashd0wn/Pheme-Ubuntu/$newVersion/web/php/www.conf

# Copy Supervisors php-fpm.conf
curl -s -o /etc/supervisor/conf.d/php-fpm.conf https://raw.githubusercontent.com/ashd0wn/Pheme-Ubuntu/$newVersion/supervisor/conf.d/php-fpm.conf
...
# Get redis.conf
curl -s -o /etc/redis/redis.conf https://raw.githubusercontent.com/ashd0wn/Pheme-Ubuntu/$newVersion/redis/redis.conf
chown redis.redis /etc/redis/redis.conf

# Get supervisor redis.conf
curl -s -o /etc/supervisor/conf.d/redis.conf https://raw.githubusercontent.com/ashd0wn/Pheme-Ubuntu/$newVersion/supervisor/conf.d/redis.conf
...
### Now it's time for Pheme
# ups :p
rm -rf /var/pheme/www_tmp/*

# Better do it as the Pheme User
if [ $yn_one = "yes" ]; then
    su pheme <<'EOF'
cd /var/pheme/www
git stash
...
# Remove
rm -f /var/pheme/installer_version.txt

# Update Version (Not needed actually this file. But leave it for now)
rm -f /var/pheme/pheme_version.txt
...
chown -R pheme.pheme /var/pheme/www*
